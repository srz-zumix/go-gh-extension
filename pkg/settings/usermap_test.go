...
// group N followed by the literal suffix "_corp".
func TestResolveSrc_BracedGroupUnderscoreSuffix(t *testing.T) {
	cm, err := settings.NewCompiledMappings(newFile(
		settings.UserMapping{Src: `(.+)`, Dst: `${1}_corp`},
	))
	require.NoError(t, err)

	dst, ok := cm.ResolveSrc("takazumi-shirayanagi")
	assert.True(t, ok)
	assert.Equal(t, "takazumi-shirayanagi_corp", dst)
}
...
func TestResolveSrc_CompactEMU_CatchAllDstSuffix(t *testing.T) {
	input := []settings.UserMapping{
		{Src: "alice", Dst: "alice_corp"},
		{Src: "bob", Dst: "bob_corp"},
	}
