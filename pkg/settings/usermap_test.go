...
// ${N}_ style references (generated by CompactEMUMappings) must correctly expand
// group N followed by the literal suffix "_dena".
func TestResolveSrc_BracedGroupUnderscoreSuffix(t *testing.T) {
	cm, err := settings.NewCompiledMappings(newFile(
		settings.UserMapping{Src: `(.+)`, Dst: `${1}_dena`},
	))
	require.NoError(t, err)

	dst, ok := cm.ResolveSrc("takazumi-shirayanagi")
	assert.True(t, ok)
	assert.Equal(t, "takazumi-shirayanagi_dena", dst)
}

// End-to-end: CompactEMUMappings output fed into NewCompiledMappings and ResolveSrc
// must correctly expand the catch-all pattern with a dst suffix.
func TestResolveSrc_CompactEMU_CatchAllDstSuffix(t *testing.T) {
	input := []settings.UserMapping{
		{Src: "alice", Dst: "alice_dena"},
		{Src: "bob", Dst: "bob_dena"},
	}
	compacted := settings.CompactEMUMappings(input)
	cm, err := settings.NewCompiledMappings(&settings.UserMappingFile{Users: compacted})
	require.NoError(t, err)

	// Both known users must resolve via the catch-all regex.
	dst, ok := cm.ResolveSrc("alice")
	assert.True(t, ok)
	assert.Equal(t, "alice_dena", dst)

	// An unknown user not in the original input must also get the suffix.
	dst, ok = cm.ResolveSrc("charlie")
	assert.True(t, ok)
	assert.Equal(t, "charlie_dena", dst)
}
...
func TestCompactEMUMappings_BothSuffix(t *testing.T) {
	// alice_corp → alice_new  ⇒  (.+)_corp → ${1}_new
	mappings := []settings.UserMapping{
	...
	assert.Equal(t, `${1}_new`, result[0].Dst)
}
