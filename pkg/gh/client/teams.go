...
// GetTeamByID retrieves a team by its ID using GraphQL
func (g *GitHubClient) GetTeamByID(ctx context.Context, org string, teamID int64) (*github.Team, error) {
	...
	var query struct {
		Organization struct {
			Team struct {
				...
				URL               githubv4.String
				MembersCount      githubv4.Int `graphql:"members { totalCount }"`
				RepositoriesCount githubv4.Int `graphql:"repositories { totalCount }"`
				ParentTeam        *struct {
					DatabaseID githubv4.Int
					ID         githubv4.String
					Name       githubv4.String
			...
			} `graphql:"team(databaseId: $teamID)"`
		} `graphql:"organization(login: $org)"`
	}

	variables := map[string]interface{}{
		"org":    githubv4.String(org),
		"teamID": githubv4.Int(teamID),
	}
	...
	teamData := query.Organization.Team

	team := &github.Team{
		ID:           github.Ptr(int64(teamData.DatabaseID)),
		NodeID:       github.Ptr(string(teamData.ID)),
		Name:         github.Ptr(string(teamData.Name)),
		Slug:         github.Ptr(string(teamData.Slug)),
		Description:  github.Ptr(string(teamData.Description)),
		Privacy:      github.Ptr(string(teamData.Privacy)),
		URL:          github.Ptr(string(teamData.URL)),
		MembersCount: github.Ptr(int(teamData.MembersCount)),
		ReposCount:   github.Ptr(int(teamData.RepositoriesCount)),
		Organization: &github.Organization{
			Login: github.Ptr(org),
		},
	}

	if teamData.ParentTeam != nil {
		team.Parent = &github.Team{
			ID:     github.Ptr(int64(teamData.ParentTeam.DatabaseID)),
			NodeID: github.Ptr(string(teamData.ParentTeam.ID)),
			Name:   github.Ptr(string(teamData.ParentTeam.Name)),
			Slug:   github.Ptr(string(teamData.ParentTeam.Slug)),
		}
	}
